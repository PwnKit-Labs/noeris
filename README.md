...
- **Fused QK-RMSNorm + RoPE prologue for Gemma 3/4.** vLLM has an experimental `enable_qk_norm_rope_fusion` pass but it is **disabled by default** due to H100 performance regression ([issue #34391](https://github.com/vllm-project/vllm/issues/34391)); the Flash Normalization paper (arXiv:2407.09577) describes algebraic optimizations. Our parameterized Triton kernel with bandit-tuned configs makes this fusion practical: **10.2–12.9× on A100**, **10.4–11.9× on H100**, and **6.06× on T4** prologue speedup across all 6 Gemma 3/4 shape buckets. The **backward pass** kernel achieves **5.1–5.2× fusion speedup on T4** (GPU-validated), making the fused prologue usable for training — not just inference (vLLM's fusion is inference-only). See [`docs/results/qk-norm-rope-fusion-speedup.md`](docs/results/qk-norm-rope-fusion-speedup.md).
