...
The system additionally covers eight other operators (matmul, rmsnorm, softmax, layernorm, cross_entropy, attention with grouped-query / sliding-window / fused QK-norm support, rotary embedding, and fused GeGLU for Gemma 2/3/4). It is evaluated across 53 shape-parameterized problems spanning 447 unit tests on NVIDIA A100 and H100. Using only curated starter configs, Noeris achieves **fast₁.₀ = 56.6% vs PyTorch eager** on our internal LLM-shape benchmark. Side-by-side apples-to-apples comparison against the upstream KernelBench harness (`cuda_event` timing + L2 flush matching the upstream methodology, fp32 `nn.Module` problems) is reported separately for transparency.
