...
All code, benchmark data, and reproduction scripts are available at https://github.com/PwnKit-Labs/noeris under the MIT License. We make seven contributions:

1. **A practical fused kernel that makes QK-norm+RoPE fusion actually work.** The Gemma 3/4 attention prologue (`Q-RMSNorm → K-RMSNorm → Q-RoPE → K-RoPE`) has prior art: vLLM has an experimental `enable_qk_norm_rope_fusion` pass (a `torch.compile` pass with a CUDA kernel) and Liao et al. describe algebraic optimizations in "Flash Normalization" (arXiv:2407.09577). However, vLLM's fusion is **disabled by default** due to H100 performance regression ([issue #34391](https://github.com/vllm-project/vllm/issues/34391)), with a reported 2-3% E2E benefit when enabled. Our parameterized Triton kernel with bandit-tuned configs (`triton_qk_norm_rope.py`, §3.2.1) beats the separated-launches baseline by 10.2–12.9× on A100 and 10.4–11.9× on H100 across six Gemma 3/4 shape buckets (60/60 correct, zero failures). Peak 1627.7 GB/s on H100 `gemma4_31b_global`. The novelty is not the fusion idea itself, but the Triton implementation with autonomous autotuning that delivers the fusion benefit reliably across hardware.
...
7. **Compiler failure analysis (§4.16).** A four-way T4 shootout shows `torch.compile(mode="reduce-overhead")` achieves 1.25× over separated PyTorch, but Noeris achieves 1.39× — an 11% gap the compiler cannot close. The compiler cannot fuse across the RMSNorm reduction boundary (row-wise `rsqrt` forces HBM materialization before RoPE). Wrapping Noeris custom ops in `torch.compile` is *counterproductive* (1.22×, slower than Noeris alone) due to tracing overhead around opaque Triton ops.
...
To isolate how much of Noeris's fusion benefit a production-grade automatic compiler can capture, we ran a four-way shootout on T4 (SM 7.5) comparing: **(A)** PyTorch separated ops (baseline, 12.39 ms), **(B)** the same ops wrapped in `torch.compile(mode="reduce-overhead")` (9.94 ms, **1.25× speedup**), **(C)** our hand-written Noeris T4-tuned Triton kernel (8.91 ms, **1.39× speedup**), and **(D)** `torch.compile` wrapping the Noeris custom ops (10.18 ms, **1.22× speedup**). Noeris wins outright with an 11% absolute gap over the compiler.
...
**Unsloth**. Has a 2.3x faster fused QK-RoPE kernel, but it does **not** include RMSNorm in the fusion — a smaller fusion scope than both vLLM's pass and our kernel.
