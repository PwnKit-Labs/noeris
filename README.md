...
- **14 parameterized Triton operators** covering the full Gemma 4 inference and training pipeline: matmul, rmsnorm (Gemma `1+w` affine), softmax (with softcap), layernorm, cross_entropy, attention (GQA + causal + sliding-window + QK-norm + YOCO KV-share), rotary (dual-base θ=10k/1M with p-RoPE), fused GeGLU, fused QK-RMSNorm+RoPE (forward), **fused QK-RMSNorm+RoPE backward** (5.1-5.2x on T4), MoE router (fused matmul+softmax+top-k), grouped GEMM (sort-free MoE expert dispatch), PLE gather (Gemma E2B/E4B per-layer embeddings), **paged-KV decode attention** (from-scratch Triton -- vLLM's is CUDA-only).
