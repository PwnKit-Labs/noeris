...
- Noeris issue #31 ("fused QK-RMSNorm+RoPE prologue") has prior art: vLLM has an experimental `enable_qk_norm_rope_fusion` pass (torch.compile + CUDA kernel), but it is **disabled by default** due to H100 performance regression (issue #34391). SGLang has `fused_qknorm_rope`, and Modular has fused RoPE+RMSNorm kernels. Our contribution is making the fusion practical via parameterized Triton kernels with autotuning, and supporting Gemma's `(1+w)` affine mode which existing fusions don't handle. With fusion off, vLLM issues 4+ separate launches in `gemma4.py:395-427`.
