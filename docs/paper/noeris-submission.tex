...
\title{One Kernel Is All You Need: Architecture-Agnostic GPU Kernel Autotuning Across Transformers and State-Space Models}
...
distinguishing properties relative to existing LLM-driven approaches:
\textbf{(1)}~parameterized kernel templates (no free-form source rewriting),
\textbf{(2)}~a shape-indexed cross-run configuration database keyed by
\texttt{(operator, shape\_bucket, hardware)} that persists winning
configurations across sessions, and \textbf{(3)}~complementary selectors---a
...
the separated-kernel-launches baseline by $10.2$--$12.9\times$ on A100 and
$10.4$--$11.9\times$ on H100, reaching 1627.7~GB/s peak throughput ($\approx$49\%
of HBM3 theoretical peak). Cross-model generalization across 19 model shapes
and 13 families confirms the fusion is architecture-general
($6.5$--$8.9\times$ on T4, $3.9$--$9.8\times$ on A100). A sliding-window
attention kernel with tile-pruning beats cuDNN FlashAttention by up to
$3.56\times$ on T4 and $6.24\times$ on A100. A bandit convergence study
demonstrates $\geq$98\% of exhaustive-search optimal throughput with just 6
configurations (12\% of the search space) in a single iteration. The system
covers 15 operators including a Mamba-3 SSM scan operator, 110+ shape buckets,
and 606 unit tests at ${\sim}\$0.01$ per iteration.

\textbf{Second headline result.} The sliding-window attention kernel with
tile-pruning beats cuDNN FlashAttention (via
\texttt{torch.nn.functional.scaled\_dot\_product\_attention}) on narrow-window
workloads: on T4, 4/8 shapes win with up to $3.56\times$ speedup; on A100,
8/8 shapes win with up to $6.24\times$ speedup. Tile-pruning skips 96--98\%
of tiles that cuDNN computes densely.

At the full-model level, a 26-layer Gemma~4 E2B forward pass achieves
$1.18\times$ end-to-end speedup on T4 and $1.43\times$ on A100 by fusing only
RMSNorm, QK-norm+RoPE, and GeGLU---operations accounting for ${<}20\%$ of
per-layer compute. Cross-hardware fused kernel latency rankings between A100
and H100 achieve Spearman $\rho = 0.907$ ($p < 10^{-7}$), validating zero-shot
cross-hardware config prediction without target-device fine-tuning.

The system runs at ${\sim}\$0.01$ per iteration. Code and data are available at
\url{https://github.com/PwnKit-Labs/noeris} (MIT License).
\end{abstract}
...
invocation, we generate kernels from a compact parameter tuple (e.g.,
...
\url{https://github.com/PwnKit-Labs/noeris} under the MIT License. We make
thirteen contributions:
...
\item \textbf{System.} A complete autonomous kernel search loop for fifteen
parameterized Triton operators, running on cloud GPUs via Modal with GitHub
Actions orchestration, at ${\sim}\$0.01$ per iteration.
...
\begin{verbatim}
 ...
 |  |   value: best_config_id, best_metric, trajectory    |      |
 |  +----------------------------+------------------------+      |
 |                               |                               |
 |                               v                               |
 |                  +-----------------------+                     |
...
\begin{table}[t]
\centering
\caption{Noeris operator registry (15 operators, 110+ shape buckets).}
\label{tab:operators}
\small
\begin{tabular}{@{}llcr@{}}
\toprule
...
(\texttt{Q-RMSNorm}$\to$\texttt{K-RMSNorm}$\to$\texttt{Q-RoPE}$\to$\texttt{K-RoPE})
is a known fusion target. vLLM has an experimental
...
(Kaggle/Colab free tier). All 15 operators validated on all three GPUs.

\textbf{Problem set.} 53 curated problems at Levels~1--3: matmul (19), rmsnorm (7),
softmax (8), layernorm (6), cross\_entropy (7), attention (6). All FP16.
...
\begin{table}[t]
\centering
\caption{Sliding-window attention vs.\ SDPA (cuDNN FlashAttention). Config: \texttt{BLOCK\_M=32, BLOCK\_N=32, warps=2, stages=2}.}
\label{tab:swa}
\small
...
All 15 operators pass correctness on T4; the issue is purely Triton PTX quality
on Turing (SM~7.5). Triton generates high-quality code for Ampere (SM~8.0) and
...
autonomous agent loop on Triton and CUDA C++, nine operator types, 5.29$\times$
on RMSNorm. Stateless across invocations. Our curated starters exceed
...
fifteen parameterized Triton operators. The headline result is a fused
...
Mengdi Zhu, et~al.
\newblock {Mirage}: Automatically generating fast {GPU} kernels without
  programming.
