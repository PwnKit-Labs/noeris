#!/usr/bin/env python3
"""Run FP8 Triton matmul autotune sweep on H100 with layout variants (v3)."""
...
CONFIGS = [
    {"layout": "kn", "BLOCK_M": 128, "BLOCK_N": 64,  "BLOCK_K": 64,  "num_warps": 8, "num_stages": 3, "GROUP_M": 8},
    {"layout": "kn", "BLOCK_M": 256, "BLOCK_N": 64,  "BLOCK_K": 128, "num_warps": 8, "num_stages": 4, "GROUP_M": 8},
    {"layout": "kn", "BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 128, "num_warps": 8, "num_stages": 4, "GROUP_M": 8},
    {"layout": "nk", "BLOCK_M": 128, "BLOCK_N": 64,  "BLOCK_K": 64,  "num_warps": 8, "num_stages": 3, "GROUP_M": 8},
    {"layout": "nk", "BLOCK_M": 256, "BLOCK_N": 64,  "BLOCK_K": 128, "num_warps": 8, "num_stages": 4, "GROUP_M": 8},
    {"layout": "nk", "BLOCK_M": 128, "BLOCK_N": 128, "BLOCK_K": 128, "num_warps": 8, "num_stages": 4, "GROUP_M": 8},
]
...
def run_one(shape, cfg):
    ...
    if cfg["layout"] == "kn":
        b_ptr = b_kn
        stride_bk = b_kn.stride(0)
        stride_bn = b_kn.stride(1)
    else:
        # Prepacked N x K layout for inference-like static weight usage.
        b_nk = b_kn.t().contiguous()
        b_ptr = b_nk
        stride_bk = b_nk.stride(1)
        stride_bn = b_nk.stride(0)
...
def main():
    ...
    for cfg in CONFIGS:
        cid = (
            f"{cfg['layout']}_bm{cfg['BLOCK_M']}_bn{cfg['BLOCK_N']}_bk{cfg['BLOCK_K']}_"
...
def _summarize(config_results: list[dict], fp16_baseline: list[dict]) -> dict:
    ...
    for cfg in config_results:
        ...
        layout = cfg.get("config", {}).get("layout", "unknown")
...
def _to_md(payload: dict) -> str:
    ...
    for shape in sorted(payload["best_by_shape"].keys()):
        ...
        lines.append(
            f"| {shape} | {row.get('layout', 'n/a')} | {row['tflops']:.4f} | {fp16_tflops:.4f} | {speedup:.3f}x | {row['ms']:.4f} | {row['max_err']:.6f} | {row['config_id']} |"
        )
    lines += [
        "",
...
def main() -> int:
    ...
    parser.add_argument("--output-json", default="docs/results/fp8-triton-matmul-autotune-h100-v3.json")
    parser.add_argument("--output-md", default="docs/results/fp8-triton-matmul-autotune-h100-v3.md")
    ...
    if not out.success:
        raise RuntimeError(f"FP8 autotune sweep v3 failed: {out.error}")
