#!/usr/bin/env python3
"""Run sliding-window showdown benchmark on Modal A100, compare with T4 results.

Tests whether Noeris Triton tile-pruning beats cuDNN FA2 on Ampere (A100),
where cuDNN has well-optimized FlashAttention-2 (unlike T4/Turing).

Usage:
    python3.11 scripts/modal_sliding_window_a100.py

Budget: ~$0.30 (A100 for <5 minutes)
"""
...
# ---------------------------------------------------------------------------
# Self-contained benchmark script sent to Modal A100
# ---------------------------------------------------------------------------
...
print(f"PyTorch {torch.__version__}, Triton {triton.__version__}, CUDA {torch.version.cuda}", flush=True)

# ============================================================================
# Shapes (same as T4 benchmark)
# ============================================================================
...
# ============================================================================
# Configs: T4-tuned + A100-tuned (larger tiles, more warps for 108 SMs)
# ============================================================================

CONFIGS = [
    # T4-tuned (small tiles, low warps) -- keep to see if they still work
    ...
    # A100-tuned (larger tiles, more warps, more pipeline stages)
...
# ============================================================================
# Triton kernel (inlined from sliding_window_showdown.py)
# ============================================================================
...
def sdpa_with_window_mask(q, k, v, window_size):
    ...
    return torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=mask, is_causal=False)
...
for shape in SHAPES:
    ...
    print(f"\nShape: {name}  (B={B}, H={H}, S={S}, D={D}, W={W})  skip={skip_pct:.1f}%", flush=True)

    q = torch.randn((B, H, S, D), device="cuda", dtype=torch.float16)
    k = torch.randn((B, H, S, D), device="cuda", dtype=torch.float16)
    v = torch.randn((B, H, S, D), device="cuda", dtype=torch.float16)

    # Baseline A: SDPA full causal
    try:
        ms_sdpa_causal = cuda_event_timer(lambda: sdpa_full_causal(q, k, v), warmup=5, trials=20)
    except Exception as e:
        print(f"  SDPA full causal FAILED: {e}", flush=True)
        ms_sdpa_causal = None

    # Baseline B: SDPA with explicit window mask
    try:
        ms_sdpa_mask = cuda_event_timer(lambda: sdpa_with_window_mask(q, k, v, W), warmup=5, trials=20)
    except Exception as e:
        print(f"  SDPA window mask FAILED: {e}", flush=True)
        ms_sdpa_mask = None

    if ms_sdpa_causal is not None:
        print(f"  SDPA-causal: {ms_sdpa_causal:.3f} ms", flush=True)
    if ms_sdpa_mask is not None:
        print(f"  SDPA-mask:   {ms_sdpa_mask:.3f} ms", flush=True)
    ...
    for cfg in CONFIGS:
        ...
        try:
            ...
            if max_err > 0.1:
                print(f"    {cid}: INCORRECT (max_err={max_err:.4f})", flush=True)
                noeris_results.append({"config": cid, "ms": None, "correct": False, "max_err": round(max_err, 5)})
                continue
            ...
            noeris_results.append({"config": cid, "ms": round(ms, 4), "correct": True, "max_err": round(max_err, 5)})
            print(f"    Noeris {cid}: {ms:.3f} ms  (err={max_err:.5f})", flush=True)
            if ms < best_noeris_ms:
                best_noeris_ms = ms
                best_noeris_cfg = cid
        except Exception as e:
            print(f"    Noeris {cid}: FAILED ({e})", flush=True)
            noeris_results.append({"config": cid, "ms": None, "correct": False, "error": str(e)[:200]})
    ...
    if best_sdpa and best_noeris_ms < float("inf"):
        speedup = round(best_sdpa / best_noeris_ms, 3)
        status = "NOERIS WINS" if wins else "SDPA faster"
        print(f"  => Best Noeris ({best_noeris_cfg}): {best_noeris_ms:.3f} ms vs SDPA ({best_sdpa_label}): {best_sdpa:.3f} ms = {speedup:.2f}x {status}", flush=True)
    if wins:
        any_win = True
...
# Output JSON for ModalBenchmarkSession to parse
payload = {
    "benchmark": "sliding_window_showdown_a100",
    "gpu": GPU_NAME,
...
def run_a100_benchmark() -> dict:
    """Send sliding-window showdown to Modal A100, return parsed results."""
    print("Launching sliding-window showdown on Modal A100-40GB...")
    print("(Budget cap: $0.50, timeout: 600s)\n")

    with ModalBenchmarkSession(gpu="A100", timeout_seconds=600, max_cost_usd=0.50) as session:
    ...
    json_start = stdout.rfind('{"benchmark"')
    if json_start < 0:
    ...
    if json_start >= 0:
        ...
        payload = {"gpu": "A100", "results": result.config_results}
    return payload


def print_comparison(t4_results: list[dict], a100_results: list[dict]):
    """Print side-by-side T4 vs A100 comparison."""
    t4_map = {r["shape"]: r for r in t4_results}
    a100_map = {r["shape"]: r for r in a100_results}
    common = sorted(set(t4_map.keys()) & set(a100_map.keys()))

    print("\n" + "=" * 110)
    print("  T4 vs A100: SLIDING-WINDOW SHOWDOWN COMPARISON")
    print("=" * 110)
    hdr = (
        f"{'Shape':<22s} {'Skip%':>5s} "
        f"{'T4 SDPA':>9s} {'T4 Noeris':>10s} {'T4 Spdup':>9s} "
        f"{'A100 SDPA':>10s} {'A100 Noeris':>12s} {'A100 Spdup':>11s} "
        f"{'Winner T4':>10s} {'Winner A100':>12s}"
    )
    print(hdr)
    print("-" * 110)
    ...
    for shape in common:
        ...
        t4_sdpa = t4.get("sdpa_causal_ms") or 0
        t4_noeris = t4.get("best_noeris_ms") or 0
        t4_spd = f"{t4.get('speedup_vs_sdpa', 0):.2f}x" if t4.get("speedup_vs_sdpa") else "N/A"
        t4_win = "NOERIS" if t4.get("noeris_wins") else "SDPA"

        a100_sdpa = a100.get("sdpa_causal_ms") or 0
        a100_noeris = a100.get("best_noeris_ms") or 0
        a100_spd = f"{a100.get('speedup_vs_sdpa', 0):.2f}x" if a100.get("speedup_vs_sdpa") else "N/A"
        a100_win = "NOERIS" if a100.get("noeris_wins") else "SDPA"

        print(
            f"{shape:<22s} {t4.get('skip_ratio_pct', 0):>5.1f} "
            f"{t4_sdpa:>9.3f} {t4_noeris:>10.3f} {t4_spd:>9s} "
            f"{a100_sdpa:>10.3f} {a100_noeris:>12.3f} {a100_spd:>11s} "
            f"{t4_win:>10s} {a100_win:>12s}"
        )

    print("-" * 110)

    # Key question summary
    a100_wins = [r["shape"] for r in a100_results if r.get("noeris_wins")]
    t4_wins = [r["shape"] for r in t4_results if r.get("noeris_wins")]
    print(f"\n  T4 shapes where Noeris wins:   {len(t4_wins)}/{len(t4_results)}  {t4_wins}")
    print(f"  A100 shapes where Noeris wins: {len(a100_wins)}/{len(a100_results)}  {a100_wins}")

    if a100_wins:
        print("\n  ** NOERIS TILE-PRUNING BEATS cuDNN FA2 ON A100! **")
    else:
        print("\n  cuDNN FA2 wins all shapes on A100 -- tile-pruning advantage is T4-specific.")
    print()


def main():
    # Load T4 results
    t4_path = REPO / "docs" / "results" / "t4-sliding-window-showdown-v2.json"
    if not t4_path.exists():
        t4_path = REPO / "docs" / "results" / "t4-sliding-window-showdown.json"
    if not t4_path.exists():
        print(f"Warning: T4 results not found, will skip comparison")
        t4_data = None
    else:
        t4_data = json.loads(t4_path.read_text())
        print(f"Loaded T4 results: {len(t4_data['results'])} shapes on {t4_data['gpu']}\n")

    # Run A100 benchmark
    a100_data = run_a100_benchmark()
    a100_results = a100_data.get("results", [])
    gpu_name = a100_data.get("gpu", "A100")
    print(f"\nA100 benchmark complete: {len(a100_results)} shapes on {gpu_name}\n")

    # Save A100 results
    out_path = REPO / "docs" / "results" / "a100-sliding-window-showdown.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(a100_data, indent=2))
    print(f"Results saved to {out_path}\n")

    # Print comparison
    if t4_data:
        print_comparison(t4_data["results"], a100_results)

    # Print final summary table (A100 only)
    print("=" * 100)
    print("  A100 RESULTS TABLE")
    print("=" * 100)
    hdr = f"{'Shape':<22} {'Skip%':>5} {'SDPA-caus ms':>12} {'SDPA-mask ms':>12} {'Noeris ms':>10} {'Config':<22} {'Speedup':>8} {'Winner':>10}"
    print(hdr)
    print("-" * 100)
    for r in a100_results:
        sdpa_c = f"{r['sdpa_causal_ms']:.3f}" if r.get('sdpa_causal_ms') else "N/A"
        sdpa_m = f"{r['sdpa_mask_ms']:.3f}" if r.get('sdpa_mask_ms') else "N/A"
        noeris = f"{r['best_noeris_ms']:.3f}" if r.get('best_noeris_ms') else "FAIL"
        cfg = r.get('best_noeris_config') or "N/A"
        spd = f"{r['speedup_vs_sdpa']:.2f}x" if r.get('speedup_vs_sdpa') else "N/A"
        winner = "NOERIS" if r.get('noeris_wins') else "SDPA"
        print(f"{r['shape']:<22} {r.get('skip_ratio_pct', 0):>5.1f} {sdpa_c:>12} {sdpa_m:>12} {noeris:>10} {cfg:<22} {spd:>8} {winner:>10}")
    print("-" * 100)
