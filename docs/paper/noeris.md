...
Noeris demonstrates a complete autonomous GPU kernel search pipeline with thirteen parameterized Triton operators — including a new fused GeGLU kernel targeting Gemma 2/3/4 MLP blocks and sliding-window local attention (§3.1) — cross-run shape-indexed configuration storage, LLM-guided proposals, and cloud GPU execution at approximately $0.01 per iteration. The system now covers 557 unit tests across 13 operators on A100, H100, and T4. On memory-bound kernels, the starting-point configurations match or exceed AutoKernel's published H100 results across RMSNorm (+120%), cross-entropy (+228%), and softmax (+85%).
